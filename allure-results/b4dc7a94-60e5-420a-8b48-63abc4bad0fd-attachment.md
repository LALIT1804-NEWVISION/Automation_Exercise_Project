...
- Name: visualTesting.spec.ts >> Automation Exercise - Visual Testing >> TC01 - Full Page Image Comparison
- Location: tests\visualTesting.spec.ts:13:9
...
Timeout: 5000ms
  ...
  Snapshot: automation-homepage.png

Call log:
  - Expect "toHaveScreenshot(automation-homepage.png)" with timeout 5000ms
  ...
  - waiting for fonts to load...
  - fonts loaded
  - Expected an image 1265px by 8819px, received 1280px by 8819px.
  - waiting 250ms before taking screenshot
  - taking page screenshot
    - disabled all CSS animations
  - waiting for fonts to load...
  - fonts loaded
  - Expected an image 1280px by 8819px, received 1265px by 8819px. 157899 pixels (ratio 0.02 of all image pixels) are different.
...
```ts
  ...
  11 |     });
  12 | 
  13 |     test('TC01 - Full Page Image Comparison', async ({ page }) => {
  14 |         
  15 |         await expect(page).toHaveTitle('Automation Exercise');
> 16 |         await expect(page).toHaveScreenshot('automation-homepage.png', {fullPage: true,maxDiffPixels: 700});
     |                            ^ Error: expect(page).toHaveScreenshot(expected) failed
  17 |     });
  18 | 
  19 |     test('TC02 - Element Comparison - Logo Validation', async ({ page }) => {
  20 |     
  21 |         const logo = page.locator('img[alt="Website for automation practice"]');
  22 |         await expect(logo).toBeVisible();
  23 |         await expect(logo).toHaveScreenshot('website-logo.png', {maxDiffPixels: 50});
  24 |     });
  25 | 
  26 | });
```